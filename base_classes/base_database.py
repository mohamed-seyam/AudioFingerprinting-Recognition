...
def get_database(database_type: str = "mysql") -> BaseDatabase:
    ...
    """
    try:
        # path is the path for the SQL handler
        # class name is the class name of the database handler
        path, db_class_name = DATABASES[database_type]
        # module is a variable that holds the path to the SQL handler
        # db_module : get nested module path
        db_module = importlib.import_module(path)
        
        # class is only the class
